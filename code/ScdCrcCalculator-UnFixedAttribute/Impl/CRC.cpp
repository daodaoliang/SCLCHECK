...
/**
  ...
  * @date     2013/07/14
  */
uint32_t CCRC::make_crc(uint32_t crc, const char *string, uint32_t size)  
{  	
 ...
 //   return (crc^m_unlXorOut);  
    ...
    unsigned int i;
    for (i = 0; i < size; i++) {
        crc = m_crc32_table[(crc ^ string[i]) & 0xff] ^ (crc >> 8);
    }
    //if (m_bRefOut)
    //{
    //    // 计算结束后，寄存器中的值“颠倒”【RefOut = true】
...
{  
	...
	sprintf(result,"%08X",make_crc(0xFFFFFFFF,buf,lSize));
	delete buf;

	return result;
}

 char*  CCRC::generate_crc(const char* buf,long lsize,char result[])
 {  
	 make_crc32_table();  
