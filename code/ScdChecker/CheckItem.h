...
public:
    ...
    // 得到当前检测项的错误类型，字符串形式
    std::string GetErrorTypeString(ErrorType etValue) const;
    ...
    // 含有子项目的条目
    // SSD类型错误添加
    bool Report(std::string itemPath, ErrorType etValue, int nRow, const std::string &strXmlText, const char *fmt, ...) const;
    // 完整性检测的条目
    bool Report(ErrorType etValue, int nRow, const std::string &strXmlText, const char *fmt, ...) const;
