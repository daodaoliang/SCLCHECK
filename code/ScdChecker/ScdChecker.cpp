...
#include "CheckItems/CrcBlankValidCheck.h"
#include "OutputMgr.h"
#include "CacheMgr.h"
...
bool CScdChecker::CheckStart()
{
    ...
    m_mapCheckItems.erase(CCrcBlankValidCheck::GetInstance()->GetItemConfig().strCheckItemKey); // calc crc need long time, put it end
...
bool CScdChecker::CheckAll()
{
    ...
    if (GetExistingVtdNav() != NULL)
    {
        ...
        // CRC valid Check
        if (CCrcBlankValidCheck::GetInstance()->IsEnabled())
        {
            if (!CCrcBlankValidCheck::GetInstance()->Check())
            {
                COutputMgr::GetInstance()->Error(
                    "检测项目%s,%s时发生错误\r\n",
