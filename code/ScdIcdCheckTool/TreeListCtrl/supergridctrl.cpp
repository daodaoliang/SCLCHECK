	...
	if(ht.iItem!=-1)
	{
		...
		if(pItem!=NULL)
		{
			...
			if(ItemHasChildren(pItem))
			{
				...
				{
					SetRedraw(0);
					int nScrollIndex = pItem->m_nIndex;
...
void CSuperGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(OnItemLButtonDown(ht))
	{
		...
		if(bSelect && ht.iItem!=-1)
		{
			...
			if (ht.iSubItem == 1 || ht.iSubItem == 2)
            {
                OnControlLButtonDown(point, ht);
            }
...
void CSuperGridCtrl::OnControlLButtonDown(CPoint point, LVHITTESTINFO& ht)
{
	if(ht.iSubItem==0)
	{
		CRect rcItem;
