...
LRESULT CCheckComboBox::OnCtlColorListBox(WPARAM, LPARAM lParam) 
{
    ...
    if (m_hListBox == NULL)
    {
        ...
        if (hWnd != 0 && hWnd != m_hWnd)
        {
            ...
            // Do the subclassing
            m_pWndProc = reinterpret_cast<WNDPROC>(GetWindowLongPtr(m_hListBox, GWLP_WNDPROC));
            SetWindowLongPtr(m_hListBox, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(ComboBoxListBoxProc));
        }
    }
...
BOOL CCheckComboBox::SetCheck(int nIndex, BOOL bCheck)
{
    ...
    if (ret != bCheck)
    {
        ...
        // Redraw the window
        Invalidate(FALSE);
		OnCheckStateChange(nIndex,bCheck);
    }

    return ret;
}

void CCheckComboBox::OnCheckStateChange(int index,BOOL bCheck)
{
